...
def get_Pmk(mk, countmk):
    if mk in countmk.keys():
        return float(countmk[mk])
        # return float(countmk[mk])/np.sum(list(countmk.values()))
    else:
        return 0


def tf_idf(wordlis, filelist, countmk, target, othertarget):  # 计算TF-IDF,并返回字典
    ...
    for i in set(wordlis):
        if i in wordinfilecount.keys():
            ...
            for j in othertarget:
                p_mk = p_mk + get_Pmk(str(i), countmk[j])
            idf = math.log(get_Pmk(str(i), countmk[target]) / p_mk + 1)
            lf = 1
            if abs(wordlis.index(str(i)) - len(wordlis) / 2) >= len(wordlis) / 6:
                lf = 1.5
            tfidf = tf * idf * lf  # 计算TF-IDF
...
def count_mk(vocabulary, filelist, target):
    countmk = []
    ...
    for i in range(len(filelist)):
        for j in filelist[i]:
            if j in countmk[target[i]]:
                countmk[target[i]][j] = countmk[target[i]][j] + 1
            else:
                countmk[target[i]][j] = 1
    for i in range(len(countmk)):
        sum = np.sum(list(countmk[i].values()))
        for j in countmk[i].keys():
            countmk[i][j] = countmk[i][j] / sum
...
if __name__ == '__main__':
    ...
    countmk = count_mk(vocabulary, filelist, news_data.target)
    # 特征词数
    ...
    i = 0
    for wordlist in filelist:
        tfidfdic = tf_idf(wordlist, filelist, countmk, news_data.target[i], set(news_data.target))  # 计算TF-IDF
    ...
    print('Test Accuracy: %.2f' % clf.score(train_X, train_y))
